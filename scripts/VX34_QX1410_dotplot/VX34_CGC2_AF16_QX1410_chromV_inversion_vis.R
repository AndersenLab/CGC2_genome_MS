...
# Loading in nucmer alignment data of AF16 cb4 to QX1410
af_cb4 <- readr::read_tsv("../../processed_data/genome_genome_alignments/AF16_QX1410.transformed.tsv", col_names = c("QXS","QXE","AFS","AFE","L1","L2","IDY","LENR","LENQ","QX_chrom","AF16_chrom")) %>%
  dplyr::mutate(inv = ifelse(AFE < AFS, "yes", "no")) %>%
  dplyr::filter(QX_chrom == "V" & AF16_chrom == "V")

# Plotting alignment of inversion on chromosome V in AF16 cb4
af_cb4_plt <- ggplot(af_cb4) +
  geom_segment(aes(x = QXS/1e6, xend = QXE/1e6, y = AFS/1e6, yend = AFE/1e6, color = inv), linewidth = 1) +
  facet_grid(~QX_chrom) +
  ...
  labs(y = "AF16 (cb4) genome coordinates (Mb)", x = "QX1410 genome coordinates (Mb)") +
...
af_cb5 <- readr::read_tsv("../../processed_data/genome_genome_alignments/AF16cb5_QX1410.transformed.tsv", col_names = c("QXS","QXE","AFS","AFE","L1","L2","IDY","LENR","LENQ","QX_chrom","AF16_chrom")) %>%
  dplyr::mutate(inv = ifelse(AFE < AFS, "yes", "no")) %>%
  dplyr::filter(QX_chrom == "V" & AF16_chrom == "V")

# Plotting alignment of inversion on chromosome V in AF16 cb5
af_cb5_plt <- ggplot(af_cb5) +
  geom_segment(aes(x = QXS/1e6, xend = QXE/1e6, y = AFS/1e6, yend = AFE/1e6, color = inv), linewidth = 1) +
  facet_grid(~QX_chrom) +
  scale_color_manual(values = c(no = "black", yes = "red")) +
  geom_vline(xintercept = 14.934651, linetype = "dashed", linewidth = 0.75, color = "blue") +
  ...
  labs(y = "AF16 (cb5) genome coordinates (Mb)", x = "QX1410 genome coordinates (Mb)") +
  scale_x_continuous(expand = c(0,0)) +
  scale_y_continuous(expand = c(0,0)) +
  coord_cartesian(xlim = c(13.5,17), ylim = c(13.5,17))
...
final_plot <- cowplot::plot_grid(
  af_cb4_plt, af_cb5_plt, cg_qx_plt, vx_qx_plt,
  nrow = 2,
  align = "h",
  axis = "tb",
  labels = c("a", "b", "c", "d"))
