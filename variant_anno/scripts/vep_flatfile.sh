...
join -t$'\t' -1 1 -2 9 -a 2 $output_dir/$(basename $gff)_sorted.tsv <(sort -t$'\t' -k9,9 $output_dir/VEP_flatFile.tsv) | \
        awk -F'\t' -v OFS='\t' '{
                if (NF == 13) {
...
sort -k1,1 -k2,2n $output_dir/VEP_joined.tsv > $output_dir/VEP_flatFile_WBGeneID.tsv
rm $output_dir/VEP_flatFile.tsv $output_dir/VEP_joined.tsv 
...
' $output_dir/VEP_flatFile_WBGeneID.tsv > $output_dir/VEP_WBGeneID_GRANTHAM_test.tsv
...
' $output_dir/VEP_WBGeneID_GRANTHAM_test.tsv > $output_dir/VEP_WBGeneID_GRANTHAM_BLOSUM_test.tsv
...
if [[ ! -f $output_dir/VEP_WBGeneID_GRANTHAM_BLOSUM_PP_final.tsv ]]; then
    echo -e "Chromosome\tPosition\tREF\tALT\tconsequence\timpact\tAA_change\tALT_samples\tbackground_variant\tpossible_EMS\ttranscript\tgeneID\tgene_name\tgrantham_score\tblosum_score\tpercent_protein" > $output_dir/VEP_WBGeneID_GRANTHAM_BLOSUM_PP_final.tsv
else 
...
tail -n +2 $output_dir/VEP_WBGeneID_GRANTHAM_BLOSUM_PP_final.tsv | awk -F',' '{print $2"_"$11}' | while read key; do 
    processed["$key"]=1
done

while IFS=$'\t' read -r chrom pos ref alt consequence impact AA strain background ems transcript_name wbgene locus grantham_score blosum_score; do  
    ...
    echo -e "$chrom\t$pos\t$ref\t$alt\t$consequence\t$impact\t$AA\t$strain\t$background\t$ems\t$transcript_name\t$wbgene\t$locus\t$grantham_score\t$blosum_score\t$pp" >> $output_dir/VEP_WBGeneID_GRANTHAM_BLOSUM_PP_final.tsv

done < $output_dir/VEP_WBGeneID_GRANTHAM_BLOSUM_test.tsv


rm $output_dir/VEP_flatFile_WBGeneID.tsv $output_dir/VEP_flatFile.tsv $output_dir/c_briggsae.QX1410_20250929.csq.gff_sorted.tsv $output_dir/VEP_WBGeneID_GRANTHAM_BLOSUM_test.tsv $output_dir/VEP_WBGeneID_GRANTHAM_test.tsv 
